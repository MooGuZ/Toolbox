...
% version by given model.
    ...
    ws.recover = reconstruction(ws.base, ws.model);
    ...
    switch lower(ws.dmode)
        ...
        case {'recover'}
            ws.hanim = imshow(ws.recover(:, :, ws.anim.progress));
...
function recover = reconstruction(anim, model)
    ...
    recover = reshape(bsxfun(@plus, model.dmatrix * data, model.bias), sz);
end
